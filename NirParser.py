...
class PandasParser:
        ...
        for file in files:
            try:
                ...
                self.logger.log_info(
                    "File %s was not found in folder %s. Stack Trace: %s." % (file, input_folder, message))
                print("File is not found.")
                exit(-2)
            except IndexError:
                self.logger.log_info("File %s is unrecognized, unable to parse." % file)
                print("File %s is unrecognized, unable to parse." % file)
                pass
    ...
    def parse(self, filename, input_folder, output_folder, output_filename):
        ...
        df = pd.read_csv(input_path, sep='\s{1,}', engine='python', usecols=[8, 9, 22, 23, 34], skiprows=3, header=None, dtype='str')
